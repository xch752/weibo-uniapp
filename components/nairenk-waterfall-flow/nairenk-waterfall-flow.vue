<template>
  <view class="flow-box" :style="'height: ' + loadingTop + 'px'">
    <view class="item" :class="left[index] == 1 ? 'left' : ''" :style="'top:' + top[index] + 'px;'"
      v-for="(item, index) in newList" :key="index" :data-index="index" @click="choose">
      <view class="pic">
        <image class="image" mode="widthFix" :src="item.image?item.image:'http://static.xch752.com/errorImg.jpg'"
          style="width: 100%; display: block;border-radius:10upx"></image>
      </view>
      <view class="content">
        <text>{{item.content}}</text>
        <view class="user">
          <image style="width: 40upx; height: 40upx; border-radius: 50%; margin-right: 10upx;"
            :src="item.creator.avatarUrl"></image>
          {{item.creator.nickname}}
        </view>
      </view>
    </view>
    <view class="loading" v-show="loading" :style="'top: ' + loadingTop + 'px'">
      <image src="http://static.xch752.com/loading.gif" style="width: 80upx; height: 80upx;"></image>
    </view>
  </view>
</template>
