<template>
  <scroll-view :style="{'height':windowHeight+'px'}" scroll-y="true" :scroll-top="scrollTop" @scroll="scroll"
    @scrolltolower="currentChange" lower-threshold="5">
    <view class="full">
      <view class="header">
        <view class="setting">
          <image style="width:48upx;height:48upx" src="../../../static/img/more.png" mode="aspectFill"
            @click.stop="moreActionSheetTap()"></image>
        </view>
      ...
      <view class="info solid-bottom" style="margin-bottom:20upx">
        ...
        <view class="flex justify-start" style="flex-flow:row wrap;padding-top: 10upx;padding-bottom:0upx;">
          <text class="bg-gray light radius text-sm" style="padding: 5upx 15upx;margin-right:15upx"
            v-if="userInfo.age">{{userInfo.age}}</text>
          <text class="bg-gray light radius text-sm" style="padding: 5upx 15upx;margin-right:15upx"
            v-if="userInfo.address">{{userInfo.address}}</text>
          <text class="bg-gray light radius text-sm" style="padding: 5upx 15upx;margin-right:15upx"
            v-if="userInfo.constellation">{{userInfo.constellation}}座</text>
          <text class="bg-gray light radius text-sm" style="padding: 5upx 15upx;margin-right:15upx"
            @click="toModifyUserInfo"><text class="cuIcon-write"></text> 编辑资料</text>
        </view>
        <view class="flex justify-start" style="flex-flow:row wrap;padding-top: 35upx;padding-bottom:10upx;">
          <text class="text-bold text-df" style="padding: 5upx 15upx;margin-right:15upx">{{data.blogCount}}<text
              class="normal">微博</text></text>
          <text @click="toFans" class="text-bold text-df"
            style="padding: 5upx 15upx;margin-right:15upx">{{data.fansCount}}<text class="normal">粉丝</text></text>
          <text @click="toAttention" class="text-bold text-df"
            style="padding: 5upx 15upx;margin-right:15upx">{{data.attentionCount}}<text class="normal">关注</text></text>
          <text @click="toCollect" class="text-bold text-df"
            style="padding: 5upx 15upx;margin-right:15upx">{{data.collectCount}}<text class="normal">收藏</text></text>
          <text @click="toLiked" class="text-bold text-df"
            style="padding: 5upx 15upx;margin-right:15upx">{{data.likeCount}}<text class="normal">点赞</text></text>
        </view>
      </view>
          ...
          if (this.objectId) {
            ...
            query.get(this.objectId).then(res => {
              ...
              if (this.userInfo.birthday) {
                this.userInfo.age = ("00" + (parseInt(this.userInfo.birthday.slice(2, 4)) - parseInt(this.userInfo
                  .birthday.slice(2, 4)) % 5).toString()).substr(-2) + '后'
                this.userInfo.constellation = this.getAstro(this.userInfo.birthday.slice(5, 7), this.userInfo
                  .birthday.slice(8, 10))
              }
        ...
        queryBlog.find().then(res => {
          ...
          res.map(item => {
            if (item.imgList) {
              ...
              item.image = 'http://static.xch752.com/errorImg.jpg'
            }
          })
          this.list = res
...
<style scoped>
  ...
  .item {
    ...
    padding-left: 50upx;
    height: 100upx;
    /*百分比占高,为了适配Pad,若不适配Pad,可以使用单位upx,更加美观*/
    width: 100%;
  }
