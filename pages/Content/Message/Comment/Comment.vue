<template>
	<view>
		<uni-nav-bar left-icon="back" @click-left="navBack" :status-bar="true" shadow="true" fixed="true" title="收到的评论"></uni-nav-bar>
		<scroll-view v-if="commentList.length === 0?false:true" :style="{'height':windowHeight+'px'}"  @scrolltolower="currentChange" scroll-y="true">
			<view class="cu-list menu-avatar">
				<view class="cu-item" v-for="(item,index) in commentList" :key="index" @click="toMicroBlog(item.commented_blog.objectId)">
					...
					<view class="action">
						<view class="blog-img" :style="{'background-image':'url('+item.commented_blog.imgList.split(',')[0]+')'}"></view>
					</view>
				</view>
			</view>
		</scroll-view>
		<view v-if="commentList.length === 0?true:false" class="flex justify-center align-center" :style="{'height':windowHeight+'px'}">
