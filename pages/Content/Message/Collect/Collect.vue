<template>
  <view>
    <uni-nav-bar left-icon="back" @click-left="navBack" :status-bar="true" shadow="true" fixed="true" title="收藏">
    </uni-nav-bar>
    <scroll-view v-if="collectList.length === 0?false:true" :style="{'height':windowHeight+'px'}"
      @scrolltolower="currentChange" scroll-y="true">
      <view class="cu-list menu-avatar">
        <view class="cu-item" v-for="(item,index) in collectList" :key="index"
          @click="toMicroBlog(item.collected_blog.objectId)">
          ...
          <view class="action">
            <view class="blog-img" :style="{'background-image':'url('+item.collected_blog.imgList.split(',')[0]+')'}">
            </view>
          </view>
        </view>
      </view>
    </scroll-view>
    <view v-if="collectList.length === 0?true:false" class="flex justify-center align-center"
