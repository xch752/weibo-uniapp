<template>
	<view>
		<uni-nav-bar left-icon="back" @click-left="navBack" :status-bar="true" shadow="true" fixed="true" title="赞"></uni-nav-bar>
		<scroll-view v-if="likesList.length === 0?false:true" :style="{'height':windowHeight+'px'}"  @scrolltolower="currentChange" scroll-y="true">
			<view class="cu-list menu-avatar">
				<view class="cu-item" v-for="(item,index) in likesList" :key="index" @click="toMicroBlog(item.liked_blog.objectId)">
					...
					<view class="content">
						...
						<view class="text-black text-xs">赞了你的微博</view>
						<view class="text-grey text-xs">{{getDateDiff(item.createdAt)}}</view>
					</view>
					<view class="action">
						<view class="blog-img" :style="{'background-image':'url('+item.liked_blog.imgList.split(',')[0]+')'}"></view>
					</view>
				</view>
			</view>
		</scroll-view>
		<view v-if="likesList.length === 0?true:false" class="flex justify-center align-center" :style="{'height':windowHeight+'px'}">
			<view style="text-align:center">
				...
				<view class="text-gray margin-top-sm">还没有收到点赞</view>
			</view>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			initData(){
				...
				query.equalTo("liked_creator","===", this.objectId)
				query.include('liked_blog','creator')
			...
			currentChange(){
				...
				query.find().then(res => {
					...
					this.pageNum ++
					this.likesList = this.likesList.concat[res]
				}).catch(err => {
					uni.hideLoading()
					uni.showToast({
						title: `加载失败${err}`,
