<template>
    <view class="full">
		<uni-nav-bar left-icon="back" @click-left="navBack" :status-bar="true" shadow="true" fixed="true" title="详情"></uni-nav-bar>
        <view class="cu-card dynamic solid-bottom margin-bottom-sm" :class="isCard?'no-card':''">
            <view class="cu-item shadow">
                <!-- 头像部分 -->
                <view class="cu-list menu-avatar">
                    <view class="cu-item">
                        <image class="cu-avatar round" style="width:80upx;height:80upx" :src="microBlogInfo.creator.avatarUrl" mode="aspectFill" :lazy-load="isLazyLoad" @click.stop="toUser(microBlogInfo.creator.objectId)"></image>
                        <view class="content flex-sub">
                            <view class="flex justify-between">
                                <text class="text-df text-bold">{{microBlogInfo.creator.nickname}}</text>
                                <image v-if="microBlogInfo.creator.objectId!=objectId" style="width:48upx;height:48upx" src="../../../static/img/more.png" mode="aspectFill" @click.stop="moreActionSheetTap(microBlogInfo)"></image>
                            </view>
                ...
                <view class="flex solid-bottom padding justify-between" style="padding-top: 0;padding-bottom:0upx;">
                    <view class="radius">
                        <image v-if="!microBlogInfo.isLike" class="card-btn" src="../../../static/img/like.png" mode="aspectFill" @click.stop="like(microBlogInfo.objectId,microBlogInfo.creator.objectId)"></image>
                        <image v-else class="card-btn" src="../../../static/img/islike.png" mode="aspectFill" @click.stop="unlike(microBlogInfo.objectId)"></image>
                        <image class="card-btn" src="../../../static/img/comment.png" style="margin-left:20upx" mode="aspectFill" @click.stop="toComment(microBlogInfo.objectId)"></image>
                        <image class="card-btn" src="../../../static/img/share.png" style="margin-left:20upx" mode="aspectFill"></image>
                    </view>
                    <view class="radius">
                        <image v-if="!microBlogInfo.isCollect" class="card-btn" src="../../../static/img/collect.png" mode="aspectFill" @click.stop="collect(microBlogInfo.objectId,microBlogInfo.creator.objectId)"></image>
                        <image v-else class="card-btn" src="../../../static/img/iscollect.png" mode="aspectFill" @click.stop="unCollect(microBlogInfo.objectId)"></image>
                    </view>
                </view>
                <!-- 标签部分 -->
                <view v-if="microBlogInfo.tag" class="flex justify-start padding" style="flex-flow:row wrap;padding-top: 0;padding-bottom:0upx;">
                    <text v-for="(tagItem,tagIndex) in microBlogInfo.tag" :key="tagIndex" class="bg-gray light round text-bold text-sm" style="padding: 5upx 30upx;margin-right:30upx">{{tagItem}}</text>
                </view>	
                <!-- 统计部分 -->
                <view class="flex solid-bottom padding-xl justify-between align-center" style="padding-top: 0;padding-bottom:0upx;">
                    ...
                    <view v-if="microBlogInfo.Geolocation.name" class="radius flex justify-center align-center" @click.stop="openLocation(microBlogInfo.Geolocation)">
                        ...
                        <text class="bg-gray round text-bold text-sm" style="padding: 5upx 30upx">{{microBlogInfo.Geolocation.name}}</text>
                    </view>
                </view>
            </view>
        </view>
