  ...
  <view>
    ...
    <scroll-view v-if="commentsList.length === 0?false:true" :style="{'height':windowHeight+'px'}"
      ...
      <view class="cu-list menu-avatar">
        <view class="cu-item" v-for="(item,index) in commentsList" :key="index" :style="{'background':item.background}"
          ...
          <view class="content">
            ...
            <view class="text-black text-sm flex">
              <view class="text-cut">
                <text v-if="item.replyUser">回复<text class="text-bold">
                    {{item.replyUser.nickname}}:</text></text>{{item.review}}
              </view>
            </view>
          </view>
          <view class="action">
    ...
    <view v-if="commentsList.length === 0?true:false" class="flex justify-center align-center"
      :style="{'height':windowHeight+'px'}">
      <view style="text-align:center">
        <image src="http://static.xch752.com/undraw_sync_files_xb3r.png" mode="aspectFit"
    ...
    <view class="cu-bar foot input" :style="[{'bottom':InputBottom+'px'}]">
      <input class="solid-bottom round" style="border:1upx solid #000000" :adjust-position="false" :focus="inputFocus"
        maxlength="300" cursor-spacing="10" :placeholder="placeholderValue" @focus="InputFocus" @blur="InputBlur"
        v-model="bodyValue" confirm-type="send" @confirm="postComment" :confirm-hold='true'></input>
      <!-- <view class="action">
