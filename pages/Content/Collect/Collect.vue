<template>
  <scroll-view :style="{'height':windowHeight+'px'}" scroll-y="true" @scrolltolower="currentChange">
    ...
    <view v-if="list.length === 0?true:false" class="flex justify-center align-center"
        ...
        queryCollect.find().then(res_collect => {
          ...
          let List = res_collect.map(item => {
            return Object.assign({}, {
              image: item.collected_blog.imgList ? item.collected_blog.imgList.split(',')[0] :
                'http://static.xch752.com/errorImg.jpg',
